...
from source.schemas.reservations import (
...
async def create_reservation(
    reservation_schema: PostReservationRequest, db: Session = Depends(get_db)
):
    sqlAlchemyReservationRepository = SqlAlchemyReservationRepository(db)
    storeReservation = StoreReservation(sqlAlchemyReservationRepository)

    reservation = Reservation(
        name=reservation_schema.name,
        reservation_date=reservation_schema.date,
        email_address=reservation_schema.email_address,
        phone_number=reservation_schema.phone_number,
    )
